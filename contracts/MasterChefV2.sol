...
// MasterChef is the master of Sele. He can make Sele and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Sele is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accTokenPerShare;   // Accumulated Seles per share, times 1e12. See below.
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = address(pool.lpToken) == address(seleAddress) ? totalStakedAmount : pool.lpToken.balanceOf(address(this));
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = IBEP20(seleAddress).balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBalance) {
            transferSuccess = IBEP20(seleAddress).transfer(_to, tokenBalance);
        } else {
            transferSuccess = IBEP20(seleAddress).transfer(_to, _amount);
        }
