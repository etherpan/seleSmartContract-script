...
contract APRIL is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // APR-BUSD pair address
    ...
    /**
     * @notice Constructs the $APR token contract.
     */
    constructor(
        uint256 _taxRate, 
        ...
        // Mints 1 APR to contract creator for initial pool setup
    ...
    function _getAPRPrice() internal view returns (uint256 _aprPrice) {
        try IOracle(oracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("Failed to fetch $APR price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _aprPrice) internal {
        if (autoCalculateTax) {
            for (
                ...
                if (_aprPrice >= taxTiersTwaps[tierId]) {
                    require(
    ...
    /**
     * @notice Operator mints APR to a recipient
     * @param _recipient The address of recipient
     * @param _amount The amount of APR to mint to
    ...
    function burnFrom(address _account, uint256 _amount) public override onlyOperator {
    ...
    function _transfer(
        ...
        if (autoCalculateTax) {
            uint256 currentAPRPrice = _getAPRPrice();
            _updateTaxRate(currentAPRPrice);
        }
